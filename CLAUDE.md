...
### `composeApp/`
Web frontend built with **Compose Multiplatform** targeting WebAssembly (`wasmJs`). Source root: `composeApp/src/webMain/kotlin/com/ulyup/tier_list/`. Entry point is `main.kt` (uses `ComposeViewport`), root UI is `App.kt`. Uses Material3 for theming.

### `server/`
**Ktor** HTTP server running on Netty (JVM). Source root: `server/src/main/kotlin/com/ulyup/tier_list/`. `Application.kt` defines routes using the Ktor routing DSL. Starts on `0.0.0.0:8080`.
